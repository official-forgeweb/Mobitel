...
export default function BrandCategories() {
  ...
  const [bookingForm, setBookingForm] = useState({
    name: '', phone: '', date: '', time: '', address: '', landmark: '', pincode: '', selectedShop: '',
  });
  ...
  const resetAll = () => {
    ...
    setBookingForm({ name: '', phone: '', date: '', time: '', address: '', landmark: '', pincode: '', selectedShop: '' });
  };

  const handleBack = () => {
    if (bookingSubmitted) { setBookingSubmitted(false); return; }
    if (serviceType) { setServiceType(null); setBookingForm({ name: '', phone: '', date: '', time: '', address: '', landmark: '', pincode: '', selectedShop: '' }); return; }
  ...
  return (
    <section className="w-full max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 relative">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold text-dark">Select Your Brand</h2>
          <p className="text-muted text-sm mt-1">Choose your phone brand to explore repair services</p>
        </div>
