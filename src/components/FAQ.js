    ...
    <section className="w-full py-20 bg-surface">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center mb-12">
          <span className="inline-block text-xs font-bold uppercase tracking-widest text-primary bg-primary-light px-3 py-1 rounded-full mb-3">Got Questions?</span>
          <h2 className="text-3xl sm:text-4xl font-black text-dark">Frequently Asked Questions</h2>
          ...
          {faqs.map((item, i) => (
            <div key={i} className={`bg-white rounded-2xl border transition-all duration-300 overflow-hidden ${open === i ? "border-primary/30 shadow-lg shadow-primary/5" : "border-border"}`}>
              ...
              <div className={`transition-all duration-300 ease-in-out ${open === i ? "max-h-40 pb-5" : "max-h-0"}`}>
                <p className="text-body text-sm leading-relaxed px-6">{item.a}</p>
              </div>
            </div>
          ))}
        </div>
