...
export default function PopularServices({ data }) {
  ...
  const services = data?.items && data.items.length > 0 ?
    data.items.map((s, i) => ({ ...defaultServices[i % defaultServices.length], ...s })) :
    defaultServices;

  const badgeText = data?.subtitle || "Premium Craftsmanship";
  const mainTitle = data?.title || "Premium repairs for every problem.";
