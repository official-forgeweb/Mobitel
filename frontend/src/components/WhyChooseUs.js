export default function WhyChooseUs({ data }) {
  ...
  const titleParts = (data?.title || "Why choose || Mobitel?").split('||');
  const title1 = titleParts[0]?.trim() || "Why choose";
  const title2 = titleParts[1]?.trim() || "Mobitel?";
  const subtitle = data?.subtitle || "The smartest way to fix it";
  const desc = data?.desc || "We combine certified expertise, genuine parts, and an incredibly seamless doorstep experience so you never have to stress about a broken device again.";
        ...
        <div className="flex flex-col md:flex-row items-stretch justify-center gap-8 lg:gap-12 relative">
          ...
          {features.map((f, i) => (
            <div key={i} className="flex-1 relative z-10 flex flex-col items-center text-center group">
              ...
              <p className="text-body text-sm leading-relaxed max-w-[250px] mx-auto">{f.desc || f.description}</p>
