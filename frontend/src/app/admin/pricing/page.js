            ...
            <header className="mb-6 flex justify-between items-center">
                ...
                <div className="flex gap-3">
                    <button onClick={async () => {
                        if (!filters.brandId || !filters.modelId) return alert('Please select a Brand and Model first from the filters below.');
                        if (!confirm(`Apply default services to ${models.find(m => m._id === filters.modelId)?.name}?`)) return;
                        const res = await fetch(`${API}/api/pricing/initialize`, {
                            method: 'POST', headers: headers(), body: JSON.stringify({ brandId: filters.brandId, modelId: filters.modelId })
                        });
                        const data = await res.json();
                        alert(`Initialized ${data.initialized} new services for this model.`);
                        fetchPricing();
                    }} className="px-5 py-2.5 bg-indigo-500 text-white rounded-xl font-bold text-sm hover:bg-indigo-600 shadow-lg shadow-indigo-200 transition-all flex items-center gap-2">
                        Initialize Defaults
                    </button>
