...
export default function ShopsAdminPage() {
    ...
    const fetchShops = async () => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5001'}/api/shops`);
    ...
    const handleDeleteShop = async (id) => {
        ...
        setDeleting(id);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5001'}/api/shops/${id}`, {
                method: "DELETE",
            });
