...
export default function RequestsAdminPage() {
    ...
    const fetchRequests = async () => {
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5001'}/api/requests`);
    ...
    const handleUpdateStatus = async (id, newStatus) => {
        setUpdating(id);
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://127.0.0.1:5001'}/api/requests/${id}`, {
                method: "PUT",
