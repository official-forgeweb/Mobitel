...
export default function ServicesPage() {
    ...
    useEffect(() => {
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/cms/services`)
            ...
            .catch(err => {
                console.error("Failed to fetch services cms", err);
                setLoading(false);
            });
    }, []);

    const hero = cmsData?.hero || {};
    const displayServices = cmsData?.servicesList?.length > 0 ? cmsData.servicesList : services;
            ...
            <section className="relative pt-24 pb-16 md:pt-32 md:pb-24 px-4 text-center z-10">
                <div className="max-w-4xl mx-auto">
                    <h1 className="text-4xl md:text-6xl font-semibold text-dark mb-6 tracking-tight leading-tight">
                        {hero.title ? hero.title : <>Expert <span className="text-primary">Mobile Repairs</span> You Can Trust.</>}
                    </h1>
                    <p className="text-body text-lg md:text-xl font-light leading-relaxed max-w-2xl mx-auto mb-10">
                        {hero.subtitle || "Fast turnaround, genuine parts, and an honest warranty. Premium care for the devices you rely on every day. Get back to what matters."}
                    </p>
                            ...
                            <div key={svc.id} className="relative group">
                                ...
                                <div className={`relative flex flex-col md:flex-row items-center gap-12 lg:gap-24 ${isEven ? "md:flex-row" : "md:flex-row-reverse"}`}>
                                    ...
                                    <div className="w-full md:w-1/2 flex justify-center relative">
                                        ...
                                        <div className={`w-64 h-64 md:w-[22rem] md:h-[22rem] rounded-[2.5rem] flex items-center justify-center ${svc.iconBg} transition-all duration-500 group-hover:shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] group-hover:-translate-y-2 relative overflow-hidden backdrop-blur-sm border border-black/5 shadow-inner`}>
                                            ...
                                            <div className="relative z-10 w-full h-full p-2 transform group-hover:scale-110 transition-transform duration-500 ease-out drop-shadow-sm flex items-center justify-center">
                                                {svc.image ? (
                                                    <img src={svc.image} alt={svc.title} className="w-full h-full object-cover rounded-[2rem] shadow-lg border border-white/20" />
                                                ) : (
                                                    svc.icon || <div className="w-16 h-16 bg-gray-200 rounded-full"></div>
                                                )}
                                            </div>
                                        </div>
                                    </div>
